...
| `omarchy plugin` | `https://github.com/yarikov/omakvn` | Standalone Quickshell bar plugin (`yarikov.omakvn`) for Omarchy 4. `setup --omarchy` installs or updates its Git checkout in `~/.config/omarchy/plugins/yarikov.omakvn/`; the main project owns the semantic IPC API and retains the legacy command-module fallback. |
