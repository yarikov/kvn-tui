...
fn handle_clipboard_text(model: &mut Model, text: &str) -> Vec<Effect> {
    match crate::clipboard::parse_share_link(text) {
        Ok(profile) => {
            ...
            model.add_profile(profile);
            model.status = crate::model::AppStatus::Info(format!("Pasted profile: {}", name));
            vec![Effect::SaveConfig]
        }
        Err(e) => {
            model.status = crate::model::AppStatus::Error(format!("Invalid URI: {}", e));
            vec![]
        }
    }
}
...
fn handle_geo_result(model: &mut Model, result: GeoResult) -> Vec<Effect> {
    ...
    match result {
        ...
        GeoResult::Error(err) => {
            model.status = crate::model::AppStatus::Error(format!("[geo] {}", err));
            vec![]
        }
    }
}
